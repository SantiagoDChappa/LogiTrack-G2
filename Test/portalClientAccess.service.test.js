...
describe('requestAccess()', () => {
    test('crea pending y envía email cuando las credenciales son válidas', async () => {
        ...
        expect(result.ok).toBe(true);
        expect(portalClientAccessPendingModel.create).toHaveBeenCalled();
...
describe('confirmAccess()', () => {
    ...
    test('rechaza token inexistente', async () => {
    ...
    test('rechaza token expirado', async () => {
        ...
        expect(portalClientAccessPendingModel.deleteByToken).toHaveBeenCalledWith('abc');
    });

    test('confirma token válido y emite sesión', async () => {
        portalClientAccessPendingModel.findByToken.mockResolvedValueOnce({
            token: 'valid',
            document: 12345678,
            email: 'cliente@test.com',
        ...
        const result = await portalClientAccess.confirmAccess('valid');
        expect(result.ok).toBe(true);
        expect(result.sessionToken).toBeTruthy();
        expect(portalClientAccessPendingModel.deleteByToken).toHaveBeenCalledWith('valid');
