...
describe('POST /shipment/import (RBAC + flujo completo)', () => {
    ...
    test('admin puede importar y ve el resumen con éxito', async () => {
        setupHappyMocks();
        ...
        expect(res.text).toContain('Envíos creados');
    ...
    test('persiste el registro en shipment_import con métricas correctas', async () => {
        setupHappyMocks();
        const app = buildApp({ id: 9, roleId: 4, fullName: 'Admin Test' });
        const csv = [HEADER, validRow()].join('\n');
